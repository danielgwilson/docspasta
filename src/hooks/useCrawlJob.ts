...
export function useCrawlJob(jobId: string | null) {
  ...
  useEffect(() => {
    ...
    const fetchJobState = async () => {
      try {
        ...
        const response = await fetch(`/api/v5/jobs/${jobId}/state`)
  ...
  useEffect(() => {
    ...
    const streamUrl = `/api/v5/jobs/${jobId}/stream`
  ...
  return {
    ...state,
    downloadUrl: state.status === 'completed' ? `/api/v5/jobs/${jobId}/download` : null
  }
}