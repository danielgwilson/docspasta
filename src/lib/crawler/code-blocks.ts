/**
 * Code block processing utilities for enhanced language detection and formatting
 */
...
export class CodeBlockHandler {
  ...
  /**
   * Detect language from element classes, data attributes, or content
   */
  public static detectLanguage(element: Element): string {
    // Check class names
   ...
   */
  public static detectLanguageFromContent(content: string): string {
    if (
  ...
  /**
   * Format code by removing excessive whitespace and normalizing indentation
   */
  ...
  /**
   * Convert code block to markdown fence with language detection
   */
  public static toMarkdownFence(content: string, element?: Element): string {
    const language = element ? this.detectLanguage(element) : this.detectLanguageFromContent(content);
    const formattedCode = this.formatCode(content);
