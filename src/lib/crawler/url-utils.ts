...
export function generateFingerprint(
  ...
  try {
    ...
    // If URL is invalid, create a hash of the raw string
    return crypto.createHash('sha1').update(url).digest('hex');
  }
}

/**
 * Generate a unique fingerprint (SHA-1) for content text.
 *
 * @param content - The content to fingerprint.
 * @returns A hex string representing the SHA-1 hash of the content.
 */
